...
# Iterate over results and gather features
succesfulGenes <- unique(results$gene)
results <- NULL

# TEMPORARY: get imputed values for GeoNet per classification label
# e.g. gnImp[gnImp$label=="LB",]$delta_cumuDNABin
gnImp <- get_imputed_GeoNet_values(dataGenesDir, vkglProtVarFileName)
...
# Save as freeze 5
resultsFreeze5 <- paste(rootDir, "data", "freeze5.csv.gz", sep="/")
